...
def main():
    ...
    for d in range(1, depth + 1):
        ...
        pqi_probs = normalize_counts(dict(Counter(
            ...
            generate_tfim_samples(J=J, h=h, z=z, theta=theta-np.pi/2, t=t, n_qubits=n_qubits, shots=shots // 3, omega=np.pi) +
            generate_tfim_samples(J=h, h=J, z=z, theta=theta+np.pi/2, t=t, n_qubits=n_qubits, shots=shots // 3, omega=np.pi)
        )), shots)
