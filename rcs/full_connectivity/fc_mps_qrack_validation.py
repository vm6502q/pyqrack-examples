...
def calc_stats(ideal_probs, exp_probs_sparse, width):
    ...
    Parameters
    ----------
    ideal_probs       : np.ndarray, shape (2^width,)
    exp_probs_sparse  : dict {int -> float}
    width             : int
    """
    n_pow   = 1 << width
    u_u     = 1.0 / n_pow
    model   = 0.5   # QV mixture: 50% ideal + 50% uniform
    ...
    xeb      = float(np.dot(ideal_c, exp_c) / np.dot(ideal_c, ideal_c)) \
               if np.dot(ideal_c, ideal_c) > 0 else 0.0
