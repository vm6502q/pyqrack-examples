...
cols = n_qubits // rows
while (rows * cols) != n_qubits:
    rows -= 1
    cols = n_qubits // rows

# Step 5: Define Qrack Backend
dev = qml.device(
    "qrack.ace", wires=n_qubits, shots=1024, long_range_rows=((rows >> 1) - 1), long_range_columns=((cols >> 1) - 1) 
)


# Step 6: Full TFIM step (repeated until ground state)
def trotter_step(lattice_shape, J, h, dt):
    def step():
        ...
        for q in range(n_qubits):
            qml.RX(h * dt / 2, wires=q)

        def add_rzz_pairs(pairs):
            for q1, q2 in pairs:
                qml.CNOT(wires=[q1, q2])
                qml.RZ(2 * J * dt, wires=q2)
                qml.CNOT(wires=[q1, q2])
