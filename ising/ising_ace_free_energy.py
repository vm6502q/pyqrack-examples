...
def main():
    ...
    t = d * dt / t1
    model = 1 - 1 / (1 + t)
    bias_0_shots = int(shots * model / (2 * n_qubits))
    bias_1_shots = int(shots * model / 4) // n_qubits
    bias_2_shots = n_qubits * (int(shots * model / 8) // nq_2)
    bias_3_shots = nq_2 * (int(shots * model / 16) // nq_3)
