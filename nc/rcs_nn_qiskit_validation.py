...
def bench_qrack(n_qubits, depth, hamming_n):
    ...
    for d in range(depth):
        ...
        experiment = QrackSimulator(n_qubits, isSchmidtDecompose=False, isStabilizerHybrid=True)
...
def main():
    if len(sys.argv) < 3:
        raise RuntimeError('Usage: python3 fc_qiskit_validation.py [width] [depth] [hamming_n]')
