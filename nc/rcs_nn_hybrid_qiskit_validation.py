...
def bench_qrack(width, depth, trials):
    ...
    for trial in range(trials):
        ...
        for d in range(depth):
            ...
            experiment_nc.set_ncrp(1.0)
            nc_circ = transpile(circ, optimization_level=3, basis_gates=basis_gates)
            experiment_nc.run_qiskit_circuit(nc_circ)

            experiment_ace = QrackSimulator(width)
            ace_circ = transpile(circ, optimization_level=3, basis_gates=QrackSimulator.get_qiskit_basis_gates())
            experiment_ace.run_qiskit_circuit(ace_circ)

            circ_aer = transpile(circ, backend=control)
            circ_aer.save_statevector()
            job = control.run(circ_aer)
            
            experiment_samples = experiment_nc.measure_shots(all_bits, (shots + 1) // 2)
            experiment_samples += experiment_ace.measure_shots(all_bits, shots // 2)

            experiment_counts = dict(Counter(experiment_samples))
            control_probs = Statevector(job.result().get_statevector()).probabilities()

            stats = calc_stats(control_probs, experiment_counts, shots, d + 1, 2048)
...
def calc_stats(ideal_probs, counts, shots, depth, hamming_n):
    ...
    # By Elara (OpenAI custom GPT)
    # Compute Hamming distances between each bitstring and its closest in control case
    min_distances = [
