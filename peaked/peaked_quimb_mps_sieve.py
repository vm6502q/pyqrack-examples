...
def run_qasm(is_sparse, file_in):
    ...
    GATE_MAP = {
        ...
        'iswap': 'ISWAP',
        'u3': 'U3',
        'u2': 'U3',  # handled specially below
        'u1': 'RZ',
    }

    # Replay the circuit onto quimb MPS
    for instruction in qc.data:
        ...
        if name not in GATE_MAP:
            raise ValueError(f"Unrecognised gate: {name}")
        ...
        quimb_c.apply_gate(GATE_MAP[name], *params, *qubits)
    ...
    next_combos = list(combinations(range(n_qubits), MAX_RADIUS + 1))
